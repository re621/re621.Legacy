...
export class QueryTracker extends SubscriptionTracker {
  ...
  public async fetchUpdatedEntries (): Promise<UpdateData> {
    ...
    const subscriptions = Array.from(this.slist.get());
    ...
    for (const [index, query] of subscriptions.entries()) {
      ...
      this.writeStatus(`&nbsp; &nbsp; - processing query #${index} [<a href="/posts?tags=${queryTags.join("+")}" target="_blank">${queryTags.length}</a>]`);

      for (const post of await E621.Posts.get<APIPost>({ "tags": queryTags, "limit": 320 }, index < 10 ? 500 : 1000))
      ...
      // There are definitely users who are subscribed to enough queries to warrant this
  ...
  protected formatSubscriptionListEntry (id: string, value: any, unsub: (name: string) => void): JQuery<HTMLElement> {
    ...
    $("<a>")
      .html(id)
      .attr({ "href": "/posts?tags=" + id.replace(" ", "+") })
      .appendTo(result);
